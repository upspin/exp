...
function Browser(page) {
	...
	var parentEl = browser.element.find(".up-parent").click(function() {
		if (atRoot()) return;

		var p = browser.path;
		var i = p.lastIndexOf("/");
		browser.navigate(p.slice(0, i))
	});

	var pathEl = browser.element.find(".up-path").change(function() {
		browser.navigate($(this).val());
	});

	browser.drawPath = function() {
		var p = browser.path;
		pathEl.val(p);
