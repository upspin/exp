...
type Watcher struct {
	...
	watchCurrent int64 // owned by watch
...
func (w *Watcher) watch() error {
	...
	events, err := w.dir.Watch(name, w.watchCurrent, done)
	if err != nil {
		return err
	}
	for {
		...
		w.watchCurrent = e.Entry.Sequence
		if e.Entry.IsDir() {
			continue
		}
...
// checkDir recursively walks the given directory and sends it file to buffer.
