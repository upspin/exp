...
//      - Prompt user for a user name for their server ("serverUserName").
...
func (s *server) startup(req *http.Request) (*startupResponse, upspin.Config, error) {
	...
	case "configureServerHostName":
		...
		st.Server.HostName = hostName // Set now because serverEndpoint uses it.
	...
	case "configureServer":
		...
		writers := []upspin.UserName{st.Server.UserName, cfg.UserName()}
		for _, s := range strings.Fields(writersList) {
			...
			if name == cfg.UserName() || name == st.Server.UserName {
				// Don't add these names twice.
				continue
			}
	...
	// with the correct step of the process. Otheriwse, if the user has not
	// specified an endpoint (including 'unassigned') in their config file,
...
func writeConfig(file string, user upspin.UserName, dir, store upspin.Endpoint, allowOverwrite bool) error {
	...
	// host@upspin.io. We'll turn it back on after more testing.
