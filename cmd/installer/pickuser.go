...
func (s *server) createUser(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		s.errorWithHeader(w, "Can't create directory "+dir, err)
		return
	}
	// Create a new subdirectory for keys in $HOMEDIR/.ssh/<username>
	keyDir := filepath.Join(s.homeDir, "upspin", userName)
	err = os.MkdirAll(keyDir, 0700)
	if err != nil {
		s.errorWithHeader(w, "Can't create directory in "+keyDir, err)
		return
	}
	...
	cmd := exec.Command("upspin", "keygen", "-where", keyDir)
