...
func main() {
	...
	if err == os.ErrNotExist { // first time
		// TODO(ehg)  not working on Windows?!!!
	...
	if i < 0 {
		log.Fatalln("couldn't find upspin user name in working directory ", getwd)
	}
	i = strings.LastIndexAny(wd[:i], "\\/")
	if i < 0 {
		log.Fatalln("unable to parse working directory", getwd)
	}
	slash := wd[i : i+1]
	wd = wd[i+1:]
	if slash != "/" {
