...
func (s *server) serveAPI(w http.ResponseWriter, r *http.Request) {
	...
	if method != "hello" && !hasConfig {
		http.Error(w, "no config", http.StatusBadRequest)
		return
	}

	// Require a valid XSRF token for any requests except "hello".
	if method != "hello" && !xsrftoken.Valid(r.FormValue("token"), s.xsrfKey, "magic", "") {
	...
	case "signup":
		if hasConfig {
			http.Error(w, "can't signup; config already present", http.StatusBadRequest)
			return
		}
