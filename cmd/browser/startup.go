...
func (s *server) startup(req *http.Request) (resp *startupResponse, cfg upspin.Config, err error) {
	...
	// link. If they have a regsitered user, but not specified an endpoint
...
// logf logs a formatted log message to $HOME/upspin/browser.log,
// or to standard error if the log file cannot be opened.
...
// newLogger initializes a log.Logger that writes to $HOME/upspin/browser.log.
func newLogger() (*log.Logger, error) {
	home, err := config.Homedir()
	if err != nil {
		return nil, err
	}
	dir := filepath.Join(home, "upspin")
