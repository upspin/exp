...
scan-store
	...
	those servers, and record a list of references to those objects.

find-garbage
	Use the results of scan-dir and scan-store operations to create a list
	of objects that are present in a store server but not referenced
	by the scanned directory servers.

delete-garbage
	Delete the objects found by find-garbage from the store server.
...
1. Run scan-store (as the store server user) to generate a list of references
   to objects in the store server.
2. Run scan-dir for each Upspin tree that stores data in the store server (as
   the Upspin users that own those trees) to generate lists of object
...
4. Run delete-garbage (as the store server user) to delete the objects in the
   find-garbage output.
`
